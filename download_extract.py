...
def download_tar_extract_images(dir_links):
    
    """
    args:
    input_dir == directory that contains the dlinks.txt from the roco_dataset folder
    ...
    """
    dlink_dir = os.path.join(dir_links, 'dlinks.txt')
    ##prepare the dataframe that contains the pmc_id column to be used to tar extraction
    df = pd.read_csv(dlink_dir, delimiter = '\t', header = None)
    ...
    ##check if the roco_data folder already exist...if not create a new one
    if not os.path.exists('roco_data'):
        os.mkdir('roco_data')
    
    ##download tar files
    for x in df['url_list']:
        wget.download(x, 'roco_data/')
    ...
    ###Finally,  lets copy the images to the original "images" folder ----print('\n Copying the new extracted images into the "images" folder \n')
    ...
    folder_list = ['roco_data', 'extract_tarfiles', 'images_folder2']
...
if __name__ == '__main__':
    args = parse_args()
    download_tar_extract_images(args.input_dir)