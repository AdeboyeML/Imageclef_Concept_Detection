...
def filtered_model_get_threshold(train_dir, val_dir, cnn_model, batch_size, learning_rate, epochs, patience, num):
    ...
    ###batch_size, epochs, learning_rate, patience, num are all changeable for hyperparameter fine-tuning###
    ...
    -- Filter out images that consist of corresponding concepts that occur at a certain number of times 
    ...
    def concepts_func(df):
        concepts_list = [item for concepts in df['concepts'] for item in concepts]
        concept_dict = dict(Counter(concepts_list).most_common())
        return concept_dict
    
    trn_concepts = concepts_func(df_train)
    val_concepts = concepts_func(df_val)
    
    def filter_data(concepts_all, df, num):
        
        ##pick out all concepts that occur once
        concept_num = [keys for keys in concepts_all if concepts_all[keys] == num]
        
        ##get the corresponding images and their concepts, for concepts that occur only once
        concept_num_dict = {}
        for x in concept_num:
            for id in range(df.shape[0]):
                if x in df['concepts'][id]:
                    concept_num_dict[df['image_name'][id]] = df['concepts'][id]
        ###print('images that contains concept occuring ', str(num) + ':', len(concept_num_dict))
        
        #check if a concept count is greater than one
        def check_concepts(value, num):
            if concepts_all[value] == num:
                return True;
            else:
                return False;
        
        #### checking all the related concepts on each image one by one, if the frequencies of all related concepts are once
        omit_concepts_imgs = {}
        for keys in concept_num_dict:
            values = concept_num_dict[keys]
            result_list = [check_concepts(value, num) for value in values if value]
            #check if all is True in result_list
            result = all(result_list)
            if result:
                print(result_list)
                ##images to drop
                omit_concepts_imgs[keys] = concept_num_dict[keys]
                
        ##images to omit
        print('Number of images to omit:', len(omit_concepts_imgs))
        
        ##get the concepts to omit
        concepts_omit = []
        for keys in omit_concepts_imgs:
            values = omit_concepts_imgs[keys]
            for val in values:
                if val not in concepts_omit:
                    concepts_omit.append(val)
        print('Number of concepts to omit:', len(concepts_omit))
        
        concepts_ck = [x for x in concepts_omit if concepts_all[x] > num]
        ##to assert that all the concepts to be omitted are not greater than the num
        if len(concepts_ck) != 0:
            print('Filtered concepts are not greater than ', str(num), 'Check the code')
            
        ##index to drop
        index_to_drop = []
        for keys in omit_concepts_imgs:
            for id in range(df.shape[0]):
                if keys == df['image_name'][id]:
                    #print(keys, df_val_trn['image_name'][id])
                    index_to_drop.append(id)
                    
        print(index_to_drop)
        
        ##drop rows i.e. images where all the related concepts occur only once or thrice (depending on "num")
    ...
    df_train = filter_data(trn_concepts, df_train, num)
    df_val = filter_data(val_concepts, df_val, num)
    
    ##create a new dir to store filtered df images
    dev_dir = os.path.join(val_dir, 'dev_' + str(num))
    if not os.path.exists(dev_dir):
        os.mkdir(dev_dir)
    if not os.path.exists(dev_dir + '/images'):
        os.mkdir(dev_dir + '/images')
        for imgs in df_val['image_path']:
            image_path = os.path.join(val_dir, imgs)
            shutil.copy(image_path, dev_dir + '/images')
            #print(imgs)
    ...
    def img_generator(df_train_concepts, train_dir, df_val_concepts, val_dir, dev_dir, batch_size, img_height, img_width):
        ...
        dev_data_gen = dev_image_generator.flow_from_directory(directory=dev_dir,
                                                               shuffle = False,
                                                               batch_size=batch_size,
                                                               target_size=(img_height, img_width), 
                                                               class_mode = None)
    ...
    train_data_gen, val_data_gen, dev_data_gen = img_generator(df_train_concepts, train_dir, df_val_concepts, 
                                                               val_dir, dev_dir, batch_size, img_height, img_width)
    ...
    def results_groundtruth_data(predictions, data_dir, val_dir, decision_threshold):
        
        #validation images
        val_images = os.listdir(os.path.join(data_dir, 'images'))
        ...
        df_cuis = pd.read_fwf(os.path.join(val_dir, 'cuis.txt'), header = None)
        cols = df_cuis.columns.tolist()[1:]
        df_cuis['concepts'] = df_cuis[cols].apply(lambda row: ';'.join(row.values.astype(str)), axis=1)
    ...
    for x in np.arange(0.1, 0.21, 0.01):
        val_gt, val_candidate = results_groundtruth_data(dev_predictions, dev_dir, val_dir, decision_threshold = x)
...
def parse_args():
    ...
    parser.add_argument('batch_size', type=int, default = 32, nargs='?', const = 16, help='Batch size for the model inputs')
    ...
    parser.add_argument('num', type=int, default = 1, nargs='?', const = 1, help='Number of corresponding concepts in images we want to FILTER out of the dataset')
    return parser.parse_args()

if __name__ == '__main__':
    args = parse_args()
    filtered_model_get_threshold(args.train_dir, args.val_dir, args.cnn_model, args.batch_size, 
                              args.learning_rate, args.epochs, args.patience, args.num)